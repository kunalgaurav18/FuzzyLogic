...
class Controller:
    ...
    def create_sets(self):
        self.distance_sets = [FuzzySets([0.0, 0.0, 0.5, 1.0], "near"),
                              FuzzySets([0.5, 1.0, 1.0, 1.5], "medium"),
                              FuzzySets([1.0, 1.5, 6.0, 6.0], "far")]
        self.speed_sets = [FuzzySets([0.0, 0.0, 0.1, 0.2], "slow"),
                           FuzzySets([0.3, 0.4, 0.45, 0.5], "medium"),
                           FuzzySets([0.45, 0.5, 0.6, 0.7], "fast")]
        self.steering_sets_rf = [FuzzySets([-3.0, -3.0, -2.0, 0], "left"),
    ...
    def defuzzification_oa(self, strength):
        ...
        for v, con in zip(strength, self.cons):
            ...
            speed += [i for i in self.speed_sets if i.label == con[0]][0].center*v
            steer += [i for i in self.steering_sets_oa if i.label == con[1]][0].center*v
        result = [speed/sum(strength), steer/sum(strength)]
        return result
    ...
    def controller(self, front_center, right_front, right_back):
        ...
        print(self.mv)
        ants_local = [self.derive_ants(mv) for mv in self.mv[:-1]]
        # print(ants_local)
        ...
        print(rf_strength)
        self.rf_res = self.defuzzification(rf_strength, self.steering_sets_rf)
        print('RF res: ', self.rf_res)
        result = self.final_calc()
        print(result)
        '''
        ants_local = [self.derive_ants(mv) for mv in self.mv]
        print("1212121 ",ants_local)
        if self.behaviour == 'RF':
            self.ants = [[x, y] for x in ants_local[0] for y in ants_local[1]]
        else:
            self.ants = [[x] for x in ants_local[0]]
        strength = self.compute_firing_strength()
        r = Rules(self.behaviour)
        print(self.mv)
        self.cons = r.rules_fired(self.ants)
        res = self.defuzzification(strength)
        return res
        '''